  ...
  const handleLinkTitleChange = (itemNew: InfoLinks) => {
    const updatedLinks = infoLinks.map((item) =>
      item.id === itemNew.id
        ? {
            ...item,
            idColor: itemNew.idColor,
            linkTitle: itemNew.linkTitle,
            url: itemNew.url,
          }
        : item
    );
    ...
    <>
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[#9fa3a5] mb-3">Add and customize your links.</p>
        </div>

        <button
          className="flex items-center gap-5 px-2 py-3 bg-black text-white rounded font-bold cursor-pointer"
